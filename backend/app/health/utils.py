...
def serialize_intraday_steps(steps: health_steps_schema.HealthStepsIntraday):
...
async def parse_and_store_health_from_file(
    token_user_id: int,
    file_path: str,
    ...
    # raise http_err
        ...
        core_logger.print_to_log(
            f"Bulk file import: Error while parsing {file_path} in parse_and_store_health_from_file - {str(err)}",
            "error",
            exc=err,
        )
        try:
...
async def parse_and_store_health_from_uploaded_file(
    ...
    try:
        ...
        if parsed_info is not None:
            created_intraday_steps = None
            created_intraday_heart_rate = []

            if file_extension.lower() == ".fit":
                if parsed_info.get("intraday_steps"):
                    # TODO: Remove extra points at the end
                    # OR convert into deltas
                    # Store step data in the database
                    created_intraday_steps = await store_intraday_steps(
                        parsed_info, db, user.id
                    )
            else:
                core_logger.print_to_log_and_console(
                    f"File extension not supported: {file_extension}", "error"
...
# TODO: Just use common parse_file
def parse_file(
    token_user_id: int,
...
async def store_intraday_steps(
    ...
    created_steps = health_steps_crud.create_health_steps_intraday(
        user_id, parsed_info["intraday_steps"], db
    )
